...
const AttendanceScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  ...
  const [allBeats, setAllBeats] = useState<DropdownItem[]>([]);
  const [beatSingle, setBeatSingle] = useState<DropdownItem | null>(null);
  ...
  const [tourPlanBlank, setTourPlanBlank] = useState('no');
  const [beatPlanBlank, setBeatPlanBlank] = useState('no');
  ...
  const findCityByName = (cityName: string, source?: 'tour' | 'beat'): DropdownItem | undefined => {
    if (source === 'beat') {
      return cities.find(
        (c) => c.value.toLowerCase().trim() === cityName.toLowerCase().trim()
      );
    } else {
      return cities.find(
  ...
  useEffect(() => {
    ...
    if (beatPlanBlank === 'yes' && tourPlanBlank === 'yes') {
      loadBeats()
    }
    if (beatPlanBlank === 'yes' && tourPlanBlank === 'no') {
      loadBeats()
    }
  }, [beatPlanBlank, tourPlanBlank]);
  ...
  useEffect(() => {
    ...
    } else if (cities.length > 0 && beats?.length > 0 && beatSingle) {
      autoSelectFromBeatPlans(selectedCities);
    }

  }, [cities, tourPlans, isPunchOutMode, beats]);
  ...
  const autoSelectFromTourPlans = () => {
    ...
    tourPlans.forEach((plan) => {
      ...
      if (
        matchingCity &&
        !autoCities.some(c => c.value === matchingCity.value) &&
        !selectedCities.some(c => c.value === matchingCity.value)
      ) {
    ...
    if (autoCities.length > 0) {
      setSelectedCities([...selectedCities, ...autoCities]);
      // setSelectedCities(autoCities);
      setTimeout(() => {
        if (beats?.length > 0 && cities.length > 0 && beatSingle) {
          autoSelectFromBeatPlans([...selectedCities, ...autoCities]);
        }
      }, 500);
    }
  };
  ...
  const autoSelectFromBeatPlans = (allObjectives: any) => {
    ...
    beats.forEach((plan) => {
      ...
      const matchingCity = findCityByName(plan.city, 'beat');

      if (
        matchingCity &&
        !autoCities.some(c => c.value === matchingCity.value) &&
        !allObjectives.some(c => c.value === matchingCity.value)
      ) {
        autoCities.push(matchingCity);
      }
    });
    ...
    if (autoCities?.length > 0) {
      ...
      setSelectedCities([...allObjectives, ...autoCities]);
    }
  };
      ...
      if (res?.data?.status === 'success' && Array.isArray(res?.data?.data) && res?.data?.data?.length > 0) {
        ...
        const formattedBeats = beatData
          ...
          .map(item => ({
            ...
            date: item?.beat_date,                    // include the date if available
            city: item.beats.city_id,                     // include city_id for potential auto-selection
            // optional: originalId: item.id,    // if you ever need the assignment row id
          }));
        setSelectedBeats(formattedBeats);
        setBeats(formattedBeats);
        setBeatSingle(formattedBeats.length > 0 ? formattedBeats[0] : null);
      } else {
        setBeats([]);
        setBeatSingle(null);
        setBeatPlanBlank('yes');
      }

    } catch (err: any) {
      setBeatError('Could not load today\'s beat plan');
      setBeatPlanBlank('yes');
      console.log(err);
    } finally {
  ...
  const getAllObjectives = (): ObjectiveItem[] => {
    const staticObjectives: ObjectiveItem[] = [
      ...
      { label: 'Retailer Visit', value: 'Retailer Visit' },
      { label: 'Mechanic Visit', value: 'Mechanic Visit' },
      { label: 'Garage Visit', value: 'Garage Visit' },
      { label: 'Workshop Visit', value: 'Workshop Visit' },
  ...
  const renderBeatPlanSection = () => {
    if (beatLoading) return <ActivityIndicator size="small" color={colors.blue} style={{ marginTop: 12 }} />;
    if (beatError) return <AppText size={13} color="red" family="InterMedium">{beatError}</AppText>;

    if (!beats?.length) {
      return (
        <View style={{ marginTop: rw(8) }}>
          <AppText size={13} color="#666">
            No Beat plan scheduled for today.
          </AppText>
        </View>
      );
    }
    ...
    const beatNames = beats
      .map((plan) => plan?.label || 'Unknown')
      .filter(Boolean); // remove empty
    const uniqueTowns = [...new Set(beatNames)];
              ...
              beats && beats?.length > 0 && (
                <View style={{ marginTop: rw(24) }}>
                  ...
                  <Pressable
                    ...
                    disabled={beatSingle ? true : false}
                    onPress={() => {
                    ...
                    >
                      {selectedBeats.map((beat) => (
                        ...
                        >
                          ...
                          <Pressable disabled={beatSingle ? true : false} onPress={() => removeBeat(beat)} hitSlop={8} style={{ marginLeft: rw(6) }}>
