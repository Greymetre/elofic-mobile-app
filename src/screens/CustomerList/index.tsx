...
const CustomerList = ({ route }: any) => {
  ...
  const fetchHierarchyUsers = async () => {
    try {
      ...
      const res = await axios.get(
        'https://elofic.fieldkonnect.io/api/getMyHierarchyUsers', // ← your endpoint 
        {
  ...
  const fetchCities = async () => {
    try {
      ...
      const res = await axios.get(
        'https://elofic.fieldkonnect.io/api/secondary-customer/cities',
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleCUrtrentCheckin = async () => {
    try {
      ...
      const res = await axios.get(
        'https://elofic.fieldkonnect.io/api/getCurrentOpenCheckin',
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const { coords, loading } = useLocationHook();
  ...
  }, [searchText, isSecondary, selectedStatus, selectedCity, selectedUser]);
          ...
          loader1 && page == 1 ? (
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => {
                if (route?.params?.beatId) {
                  return (
                    <SecondaryCustomerCard currentLat={coords?.latitude} currentLng={coords?.longitude} locationError={locationError} item={item?.customer} navigation={navigation} index={index} isPunchedIn={isPunchedIn} onCheckInPress={() => {
                      handleCUrtrentCheckin()
                    }} />
                  )
                } else if (isSecondary) {
                  return (
                    <SecondaryCustomerCard currentLat={coords?.latitude} currentLng={coords?.longitude} locationError={locationError} item={item} navigation={navigation} index={index} isPunchedIn={isPunchedIn} onCheckInPress={() => {
                      handleCUrtrentCheckin()
                    }} />
                  )
                } else {
                  return (<CustomerCard currentLat={coords?.latitude} currentLng={coords?.longitude} locationError={locationError} item={item} navigation={navigation} index={index} isPunchedIn={isPunchedIn} onCheckInPress={() => {
                    handleCUrtrentCheckin()
                  }} />)
                }
              }}
              ListHeaderComponent={() => {
                if (currentCheckin?.entity_details && currentCheckin?.entity_type == "secondary_customer") {
                  return (
                    <SecondaryCustomerCard currentLat={coords?.latitude} currentLng={coords?.longitude} locationError={locationError} item={currentCheckin?.entity_details} navigation={navigation} index={-1} isPunchedIn={isPunchedIn} type={currentCheckin} />
                  )
                } else if (currentCheckin?.entity_details && currentCheckin?.entity_type == "distributor") {
                  return (
                    <CustomerCard currentLat={coords?.latitude} currentLng={coords?.longitude} locationError={locationError} item={currentCheckin?.entity_details} navigation={navigation} index={-1} isPunchedIn={isPunchedIn} type={currentCheckin} />
                  )
                }
              }}
