...
import { useGetBeatListApi, useGetCityListApi, useGetDistrictListApi, useGetPincodeListAPi, useGetStateListApi, useGetSuperVisorListApi } from '../../api/query/CustomerApi';
...
const AddCustomer = ({ navigation, route }: any) => {
  ...
  const [formData, setFormData] = useState({
    ...
    last12MonthsSales: '',   
    assignedSalesExecutives: user?.id ? String(user.id) : '',
  ...
  const { mutateAsync: mutatePincodeList } = useGetPincodeListAPi();
  const { mutateAsync: mutateSuperVisorList } = useGetSuperVisorListApi();
  const { mutateAsync: getBeats } = useGetBeatListApi();


  const [salesExecutiveList, setSalesExecutiveList] = useState<any[]>([]);
  ...
  const loadUserList = async () => {
    try {
      ...
      const response = await fetch(
        `${BASE_URL}api/tour/userlist?global=true`,
        {
          method: 'GET',
          headers: {
      ...
      if (json?.status === 'success') {
        ...
        // Assigned sales executive always logged-in user
      ...
      console.log('User list API error', error);
    }
  };
  ...
  const ImageUploadBox = ({ label, value, onChange, required = false }: ImageUploadProps) => {
    ...
    if (value?.uri) {
      ...
      switch (fieldName) {
        case 'shopImage':
          displayImage = data?.shop_image ? `${BASE_URL}public/storage/${data.shop_image}` : null;
          console.log(displayImage, 'data?.shop_imagedata?.shop_image')
          break;
        case 'profileImage':
          displayImage = data?.profile_image ? `${BASE_URL}public/storage/${data.profile_image}` : null;
          break;
        case 'cancelledCheque':
          displayImage = data?.cancelled_cheque ? `${BASE_URL}public/storage/${data.cancelled_cheque}` : null;
          break;
        case 'mouDocument':
          displayImage = data?.mou_file ? `${BASE_URL}public/storage/${data.mou_file}` : null;
          break;
        case 'additionalDocument':
          const parsed = JSON.parse(data?.documents);
          if (Array.isArray(parsed)) {
            if (parsed.length > 0) {
              displayImage = `${BASE_URL}public/storage/${parsed[0]}`;
            } else {
              displayImage = null
            }
          } else {
  ...
  const handleGetPincodeList = async (value: any, status: any) => {
    try {
      const res = await mutatePincodeList(value);
      if (res?.data?.status == "success") {
        const formatted = res?.data?.data?.map((item: any) => ({
          label: item.pincode,
          value: String(item.pincode_id),          // better to use ID as value
          // value: item.state_name,     // ← if your backend expects name instead
        }));
        if (status == "billing") {
          setPincodesBilling(formatted);
        }
        if (status == "shipping") {
          setPincodesShipping(formatted)
        }
      }
    } catch (error) {
      console.log("get challenges by user error:", error);
  ...
  const getBillingAddressCount = () => {
    ...
    if (formData.billingPincode) count++;
    return count;
  };

  const getShippingAddressCount = () => {
    return formData.shippingAddresses.filter(
      (item) => item.trim() !== ''
    ).length;
  };
  ...
  const isFormValid =
    ...
    (
      ...
      formData.shippingAddresses.some(item => item.trim() !== '')
    ) &&
  ...
  const removeShippingAddress = (index: number) => {
    const updated = formData.shippingAddresses.filter((_, i) => i !== index);

    setFormData(prev => ({
      ...prev,
      shippingAddresses: updated.length ? updated : [''],
    }));
  };
  ...
  const areaTerritoryOptions = [
    ...
    { label: 'Test Central', value: 'test-central' },
    // ...
  ];

  const beatRouteOptions = [
    { label: 'Vijaynagar-452010', value: 'vijaynagar-452010' },
    { label: 'Vijaynagar-452001', value: 'vijaynagar-452001' },
    { label: 'Arera Colony-462016', value: 'arera-colony-462016' },
    { label: 'Kolar Road-462042', value: 'kolar-road-462042' },
    // ...
  ];
  ...
  const prepareApiPayload = () => {
    ...
    payload.append(
      'shipping_address',
      JSON.stringify(
        formData.shippingAddresses.filter(
          item => item.trim() !== ''
        )
      )
    );
    ...
    if (formData.gpsLocation) {
      payload.append('gps_location', formData.gpsLocation || '');
    }
  ...
  useEffect(() => {
    if (isEditMode && route.params.customer) {
      ...
      setFormData({
        ...
        billingCountry: data.billing_country == '1' ? "IN" : 'IN',
      ...
      if (data.billing_state) {
        handleGetDistrictList(data.billing_state, "billing",);
      }
      if (data.billing_district) {
        handleGetCitiesList(data.billing_district, "billing");
      }
      if (data.billing_city) {
        handleGetPincodeList(data.billing_city, "billing");
      }
  ...
  const fetchCurrentLocation = () => {
    ...
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        console.log(coordsString, 'coordsStringcoordsString')
        handleChange('gpsLocation', coordsString);
      ...
      (error) => {
        ...
        setUseCurrentLocation(false); // auto turn off switch on error
        Toast.show({
          type: 'error',
  ...
  // ── useEffect to react to switch toggle ──
  useEffect(() => {
      ...
      >
        ...
        <AccordionSection title="Address & Location Information">
          ...
          <View style={{ alignItems: 'center', marginBottom: 16 }}>
            ...
            >
              {formData.sameAsBilling
                ...
                : `Shipping ${getShippingAddressCount()} completed`}
            </AppText>
          </View>
          ...
          <Dropdown
            ...
            selectedTextStyle={{ color: colors.black, fontSize: 14 }}
            inputSearchStyle={{ height: 40, fontSize: 14 }}
            data={statesList}           // ← your API-fetched array
            search
            maxHeight={300}
            labelField="label"
            valueField="value"
            ...
            onChange={(item) => {
              console.log(formData.billingState, 'formData.billingState')
              handleChange('billingState', item.value);
              if (formData.sameAsBilling) handleChange('shippingState', item.value);

              // Important: reset downstream fields when state changes
              handleChange('billingDistrict', '');
              handleChange('billingCity', '');
              handleChange('billingPincode', '');
              if (formData.sameAsBilling) {
                handleChange('shippingDistrict', '');
                handleChange('shippingCity', '');
                handleChange('shippingPincode', '');
              }
              console.error("calllll api")
              // → Call your API to load districts for this state
              handleGetDistrictList(item.value, "billing");
            }}
            renderRightIcon={() => <ArrowDownIcon />}
          />
          ...
          <Dropdown
            ...
            selectedTextStyle={{ color: colors.black, fontSize: 14 }}
            inputSearchStyle={{ height: 40, fontSize: 14 }}
            data={districtsBilling}     // ← loaded based on selected state
            search
            maxHeight={300}
            labelField="label"
            valueField="value"
            ...
            onChange={(item) => {
              handleChange('billingDistrict', item.value);
              if (formData.sameAsBilling) handleChange('shippingDistrict', item.value);
              ...
              // → Call your API to load cities for this district
              // fetchCities(formData.billingState, item.value);
              handleGetCitiesList(item.value, "billing");
            }}
            renderRightIcon={() => <ArrowDownIcon />}
          />
          ...
          <Dropdown
            ...
            selectedTextStyle={{ color: colors.black, fontSize: 14 }}
            inputSearchStyle={{ height: 40, fontSize: 14 }}
            data={citiesBilling}        // ← loaded based on selected district
            search
            maxHeight={300}
            labelField="label"
            valueField="value"
            ...
            onChange={(item) => {
              handleChange('billingCity', item.value);
              if (formData.sameAsBilling) handleChange('shippingCity', item.value);

              handleChange('billingPincode', '');
              if (formData.sameAsBilling) handleChange('shippingPincode', '');

              // → Call your API to load pincodes for this city
              // fetchPincodes(formData.billingState, formData.billingDistrict, item.value);
              handleGetPincodeList(item.value, "billing");
          ...
          <Dropdown
            ...
            selectedTextStyle={{ color: colors.black, fontSize: 14 }}
            inputSearchStyle={{ height: 40, fontSize: 14 }}
            data={pincodesBilling}      // ← loaded based on selected city
            search
            maxHeight={300}
            labelField="label"
            valueField="value"
            ...
            onChange={(item) => {
              handleChange('billingPincode', item.value);
              if (formData.sameAsBilling) handleChange('shippingPincode', item.value);
            }}
            renderRightIcon={() => <ArrowDownIcon />}
          />
            ...
            <Switch
              ...
              onValueChange={(checked) => {
                ...
                if (checked) {
                  ...
                  setFormData(prev => ({
                    ...prev,
                    shippingAddresses: [prev.billingAddressLine1],
                  }));
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View style={{ flex: 1 }}>
                      <CustomTextInput
                        ...
                        value={address}
                        onChangeText={(text) =>
                          handleShippingAddressChange(index, text)
                        }
                      />
                    </View>
                    ...
                    {formData.shippingAddresses.length > 1 && (
                      <TouchableOpacity
                        onPress={() => removeShippingAddress(index)}
                        style={{
                          marginLeft: 10,
        ...
        <AccordionSection title="Business & Operational Information">
          ...
          <Dropdown
            ...
            selectedTextStyle={{ color: colors.black, fontSize: 14 }}
            inputSearchStyle={{ height: 40, fontSize: 14 }}
            data={beats} // ← fill from your logic / API
            search
            maxHeight={300}
            labelField="label"
            valueField="value"
            ...
            value={parseInt(formData.beatRoute)}
