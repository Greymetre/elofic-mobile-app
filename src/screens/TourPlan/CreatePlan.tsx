...
const CreatePlan: React.FC = ({ navigation, route }: any) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const res = await fetch('https://elofic.fieldkonnect.io/api/tour/userlist', {
  ...
  const fetchDistricts = async () => {
    ...
    try {
      const res = await fetch('https://elofic.fieldkonnect.io/api/userDistrictList', {
        headers: { Authorization: `Bearer ${token}` },
      });
      const json = await res.json();
  ...
  const fetchCitiesForDistrict = async (districtId: string | number) => {
    ...
    try {
      const res = await fetch(
        `https://elofic.fieldkonnect.io/api/userCitiesByDistrict?district_id=${districtId}`,
        {
          headers: { Authorization: `Bearer ${token}` }, 
        }
      );
  ...
  const openObjectiveSheet = (rowId: string) => {
    ...
    const fixedOptions = ["Distributor Visit", "Retailer Visit", "Mechanic Visit", "Garage Visit", "Workshop Visit", "Other"];
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      ...
      >
        <View style={{ paddingHorizontal: 20, paddingBottom: 0, paddingTop: 20 }}>
          ...
          <ScrollView style={{  }}>
            {["Distributor Visit", "Retailer Visit", "Mechanic Visit", "Garage Visit", "Workshop Visit", "Other"].map(opt => (
              <TouchableOpacity
                key={opt}
                activeOpacity={0.7}
                style={{
